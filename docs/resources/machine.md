...
Set `serialize_upgrades = true` on worker resources to upgrade them one at a time,
including workers managed with `for_each`. Omitted or `false` preserves concurrent
upgrades.
...
Changing only this option does not trigger an upgrade. Keep explicit control-plane
